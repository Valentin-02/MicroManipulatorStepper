    ...
    def calibrate_angle_mapping(self, positions_list, feedrate=10.0):
        ...
        for idx, (x, y, z) in enumerate(positions_list):
            ...
            if res != SerialInterface.ReplyStatus.OK:
                print(f"[Angle Calibration] Error moving to position: {res}")
                return False
            ...
            if res != SerialInterface.ReplyStatus.OK:
                print(f"[Angle Calibration] Error waiting for stop: {res}")
                return False
            
            # Lese die aktuellen Winkel
            angles = self.read_encoder_angles()
            if len(angles) == 0:
                print(f"[Angle Calibration] Error reading encoder angles")
                return False
    ...
    def move_to_current_angle(self, feedrate=10.0):
        ...
        if len(self.x_angle_to_position_mapping) == 0:
            if os.path.exists("config/calibration_mapping.json"):
                ...
                self.x_angle_to_position_mapping = {float(k): v for k, v in mapping_table["x"].items()}
                self.y_angle_to_position_mapping = {float(k): v for k, v in mapping_table["y"].items()}
                self.z_angle_to_position_mapping = {float(k): v for k, v in mapping_table["z"].items()}
            else:
                print("[move_to_current_angle] No calibration data available. Run calibrate_angle_mapping first.")
        ...
        if len(current_angles) == 0:
            print("[move_to_current_angle] Error reading encoder angles")
            return SerialInterface.ReplyStatus.ERROR
